...
@Injectable()
export class BscPollingBlockService {
  private detectInfo = { flag: false, blockNumber: 0 };

  private readonly logger = new Logger(BscPollingBlockService.name);
  ...
  onModuleInit() {
    ...
    this.rpcUrl = process.env.BSC_PROVIDER_URL;
  ...
  async init() {
    ...
    if (!blockSync) {
      blockSync = await this.blockSyncRepository.create({
        rpcUrl: this.rpcUrl,
        chain: SupportedChain.BSC.name,
        lastSync: await this.getBlockNumber(),
      });
    }
    // checking force latest block config
    const startBlockConfig = process.env.BSC_START_BLOCK_CONFIG;
    if (startBlockConfig === 'latest') {
      ...
      this.logger.warn(
        'force running start block from config ' + process.env.BSC_START_BLOCK,
      );
      this.updateLastSyncBlock(parseInt(process.env.BSC_START_BLOCK));
      // if we start at config block, we suppose that we already scan at (config block - 1)
      this.detectInfo.blockNumber = parseInt(process.env.BSC_START_BLOCK) - 1;
    } else {
      ...
      this.detectInfo.blockNumber =
        blockSync.lastSync + SupportedChain.BSC.confirmationBlock;
    }
    this.detectInfo.flag = false;
    this.addCronJob('BscPollingBlock', CronExpression.EVERY_5_SECONDS);
  }
      ...
      try {
        ...
        this.workerClient.emit(
          ...
          new BlockTransportDto(
            blockNumber - SupportedChain.BSC.confirmationBlock,
            true,
          ),
        );
