...
@Injectable()
export class MantlePollingBlockService {
  private detectInfo = { flag: false, blockNumber: 0 };

  private readonly logger = new Logger(MantlePollingBlockService.name);
  ...
  async init() {
    ...
    if (startBlockConfig === 'latest') {
      ...
      this.detectInfo.blockNumber =
        parseInt(process.env.MANTLE_START_BLOCK) - 1;
    } else {
    ...
    this.addCronJob('MantlePollingBlock', CronExpression.EVERY_5_SECONDS);
  }
