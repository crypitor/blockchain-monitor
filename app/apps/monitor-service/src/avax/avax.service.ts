import { AvaxEventHistoryRepository } from '@app/shared_modules/event_history/repositories/event_history.repository';
import {
  EventHistory,
  WebhookType,
} from '@app/shared_modules/event_history/schemas/event_history.schema';
import { AvaxMonitorAddressRepository } from '@app/shared_modules/monitor/repositories/monitor.address.repository';
...
export class AvaxService {
  private readonly logger = new Logger(AvaxService.name);

  @Inject()
  private readonly monitorAddressRepository: AvaxMonitorAddressRepository;
  ...
  @Inject()
  private readonly eventHistoryRepository: AvaxEventHistoryRepository;

  async findEthAddress(address: string): Promise<MonitorAddress[]> {
    return this.monitorAddressRepository.findByAddress(address);
  }
