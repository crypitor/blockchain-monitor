import { AvaxBlockHistoryRepository } from '@app/shared_modules/block_history/repositories/block_history.repository';
...
@Injectable()
export class AvaxWorker {
  private readonly logger = new Logger(AvaxWorker.name);
  rpcUrl: string;
  ...
  @Inject()
  private readonly blockHistoryRepository: AvaxBlockHistoryRepository;

  constructor() {
    if (process.env.AVAX_PROVIDER_URL) {
  ...
  private async emitLog(logs: Log[], confirm: boolean): Promise<void> {
    ...
    logs.forEach((event) => {
      if (event.topics.length === 3) {
        ...
        this.monitorClient.emit(TopicName.AVAX_ERC20_TRANSFER, {
          event: event,
          confirm: confirm,
        });
      } else if (event.topics.length === 4) {
        this.logger.debug(`emit event on ERC721 ${JSON.stringify(event)}`);
        this.monitorClient.emit(TopicName.AVAX_ERC721_TRANSFER, {
          event: event,
          confirm: confirm,
        });
      }
  ...
  private async emitNativeTransaction(
    ...
    block.prefetchedTransactions.forEach((transaction) => {
      ...
      this.monitorClient.emit(TopicName.AVAX_NATIVE_TRANSFER, {
  ...
  private async saveBlockHistory(
    ...
    if (isError && retry < 3) {
      ...
      this.workerClient.emit(TopicName.AVAX_DETECTED_BLOCK, {
        key: 'error',
