import { MantleBlockHistoryRepository } from '@app/shared_modules/block_history/repositories/block_history.repository';
...
export class MantleWorker {
  ...
  @Inject()
  private readonly blockHistoryRepository: MantleBlockHistoryRepository;

  constructor() {
    if (process.env.MANTLE_PROVIDER_URL) {
      this.rpcUrl = process.env.MANTLE_PROVIDER_URL;
      this.provider = new ethers.JsonRpcProvider(
        process.env.MANTLE_PROVIDER_URL,
        null,
        { staticNetwork: true },
      );
    }
  ...
  private async emitLog(logs: Log[], confirm: boolean): Promise<void> {
    ...
    logs.forEach((event) => {
      if (event.topics.length === 3) {
        ...
        this.monitorClient.emit(TopicName.MANTLE_ERC20_TRANSFER, {
          event: event,
          confirm: confirm,
        });
      } else if (event.topics.length === 4) {
        this.logger.debug(`emit event on ERC721 ${JSON.stringify(event)}`);
        this.monitorClient.emit(TopicName.MANTLE_ERC721_TRANSFER, {
          event: event,
          confirm: confirm,
        });
      }
  ...
  private async emitNativeTransaction(
    ...
    block.prefetchedTransactions.forEach((transaction) => {
      ...
      this.monitorClient.emit(TopicName.MANTLE_NATIVE_TRANSFER, {
  ...
  private async saveBlockHistory(
    ...
    if (isError && retry < 3) {
      ...
      this.workerClient.emit(TopicName.MANTLE_DETECTED_BLOCK, {
        key: 'error',
