import { BscBlockHistoryRepository } from '@app/shared_modules/block_history/repositories/block_history.repository';
...
@Injectable()
export class BscWorker {
  private readonly logger = new Logger(BscWorker.name);
  rpcUrl: string;
  ...
  @Inject()
  private readonly blockHistoryRepository: BscBlockHistoryRepository;

  constructor() {
    if (process.env.BSC_PROVIDER_URL) {
  ...
  private async emitLog(logs: Log[], confirm: boolean): Promise<void> {
    ...
    logs.forEach((event) => {
      if (event.topics.length === 3) {
        ...
        this.monitorClient.emit(TopicName.BSC_ERC20_TRANSFER, {
          event: event,
          confirm: confirm,
        });
      } else if (event.topics.length === 4) {
        this.logger.debug(`emit event on ERC721 ${JSON.stringify(event)}`);
        this.monitorClient.emit(TopicName.BSC_ERC721_TRANSFER, {
          event: event,
          confirm: confirm,
        });
      }
  ...
  private async emitNativeTransaction(
    ...
    block.prefetchedTransactions.forEach((transaction) => {
      ...
      this.monitorClient.emit(TopicName.BSC_NATIVE_TRANSFER, {
  ...
  private async saveBlockHistory(
    ...
    if (isError && retry < 3) {
      ...
      this.workerClient.emit(TopicName.BSC_DETECTED_BLOCK, {
        key: 'error',
