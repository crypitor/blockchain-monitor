...
export class MantleWorker {
  ...
  constructor(
    ...
    this.provider = new ethers.JsonRpcProvider(
      process.env.MANTLE_WEB3_PROVIDER_URL,
    );
  ...
  async initWorker() {
    ...
    if (!blockSync) {
      blockSync = await this.blockSyncService.create({
        rpcUrl: this.rpcUrl,
        lastSync: parseInt(process.env.MANTLE_START_BLOCK),
      });
    }
  ...
  @Cron(CronExpression.EVERY_10_SECONDS, {
    disabled: process.env.MANTLE_DISABLE === 'true',
  })
  async detect() {
    ...
    while (lastDetectedBlock <= latestDetectedBlockNumber) {
      try {
        if (lastDetectedBlock <= latestDetectedBlockNumber - 1000) {
          this.logger.debug(
            'DETECT scanning block from ' +
              lastDetectedBlock +
              ' to ' +
              (lastDetectedBlock + 1000),
          );
          // Retrieve transfer event the 1000 block's logs
          const logs = await this.provider.getLogs({
            fromBlock: lastDetectedBlock,
            toBlock: lastDetectedBlock + 1000,
            topics: [
          ...
          this.logger.debug(
            'DETECT  scanning block from ' +
              lastDetectedBlock +
              ' to ' +
              latestDetectedBlockNumber,
          );
          // Retrieve transfer event the 1000 block's logs
          const logs = await this.provider.getLogs({
            fromBlock: lastDetectedBlock + 1,
            toBlock: latestDetectedBlockNumber,
            topics: [
              '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef',
            ],
          });
    ...
    while (lastConfirmedBlock <= latestConfirmedBlockNumber) {
      try {
        if (lastConfirmedBlock <= latestConfirmedBlockNumber - 1000) {
          this.logger.debug(
            'CONFIRM scanning block from ' +
              lastConfirmedBlock +
              ' to ' +
              (lastConfirmedBlock + 1000),
          );
          // Retrieve transfer event the 1000 block's logs
          const logs = await this.provider.getLogs({
            fromBlock: lastConfirmedBlock,
            toBlock: lastConfirmedBlock + 1000,
            topics: [
              '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef',
            ],
          });
          ...
          const logs = await this.provider.getLogs({
            fromBlock: lastConfirmedBlock,
            toBlock: latestConfirmedBlockNumber,
            topics: [
              '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef',
            ],
          });
  ...
  async handleNftTransfer(event: Log, action: 'detected' | 'confirmed') {
    ...
    if (toWallet) {
      ...
      this.logger.debug(
        action + ' nft transfer IN with database: ' + JSON.stringify(body),
      );
    }
  }
